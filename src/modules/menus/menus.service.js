import { toSentenceCase } from 'src/utils/formatters.js';
...
export async function getMenuById({ id }) {
    return await globalPrisma.menu.findUnique({ where: { id }, include: { children: true } });
}
...
export async function getNavigationMenus({ roleId }) {
    ...
    const menus = await globalPrisma.menu.findMany({
        ...
        select: {
            ...
            children: {
                where: { isActive: true },
                select: { id: true, name: true, url: true, icon: true, portal: true, sortOrder: true },
                orderBy: { sortOrder: 'asc' },
            },
        },
        orderBy: { sortOrder: 'asc' },
    ...
    const filteredMenus = menus
        ...
        const filteredChildren = menu.children
            ...
            return {
                ...
                permissions: childPermissions.map((p) => ({ value: p, label: toSentenceCase(p) })),
            };
        });
        if (menuPermission || filteredChildren.length > 0) {
            return {
                ...
                permissions: (menuPermission ?? []).map((p) => ({ value: p, label: toSentenceCase(p) })),
